    ...
    def __init__(self, hyper_para: ri.basic_input, infer_mode=False, infer_dps=False) -> None:
        ...
        if not infer_mode:
            ...
            if hasattr(hyper_para, "coor_path"):
                ...
                # 支持合并配置文件中的cnf_dims参数
                expected_dims = getattr(hyper_para, 'cnf_dims', hyper_para.dims)
                assert (
                    coord.shape[-1] == expected_dims
                ), f"coordinate dimension ({coord.shape[-1]}) is not consistent with dims in yaml file ({expected_dims})"
            else:
                Warning(
        ...
        if exists(f"{hyper_para.save_path}/normalizer_params.pt"):
            ...
            self.in_normalizer.fit_normalize(
                coord if hyper_para.lumped_latent else coord.flatten(0, expected_dims-1) 
            )
            if extra_siren_in_flag:
                ...
                self.out_normalizer.fit_normalize(
                    fois if hyper_para.lumped_latent else fois.flatten(0, expected_dims)
                )
            if extra_siren_in_flag:
        ...
        if "kwargs" not in hyper_para.NF:
            ...
            self.nf: torch.nn.Module = getattr(nf_networks, hyper_para.NF["name"])(
                in_coord_features=expected_dims if not extra_siren_in_flag else expected_dims+1,
        ...
        if not infer_mode:
            ...
            self.latents = LatentContainer(
                self.N_samples, hyper_para.hidden_size, expected_dims, hyper_para.lumped_latent
            )
...
if __name__ == "__main__":
    ...
    # 检查是否是合并配置文件，如果是则提取CNF相关参数
    if hasattr(hp, 'cnf_multiGPU'):
        print("检测到合并配置文件，提取CNF相关参数...")
        hp.multiGPU = hp.cnf_multiGPU
        hp.batch_size = hp.cnf_batch_size
        hp.test_batch_size = hp.cnf_test_batch_size
        hp.epochs = hp.cnf_epochs
        hp.save_every = hp.cnf_save_every
        hp.save_path = hp.cnf_save_path
        hp.dims = hp.cnf_dims
        hp.loss_fn = hp.cnf_loss_fn
        hp.test_criteria = hp.cnf_test_criteria
        hp.lr = hp.cnf_lr
    ...
    # 保存潜在向量
    all_latents = mytrainer.latents.latents.detach().cpu().numpy()
    output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Input", "cnf_model", "latent_vector")
    os.makedirs(output_dir, exist_ok=True)

    # 按时间顺序划分：前80%训练，中间10%验证，最后10%测试
    N_samples = all_latents.shape[0]
    n_train = int(N_samples * 0.8)
    n_valid = int(N_samples * 0.1)

    train_data = all_latents[:n_train]                          # 前80%
    valid_data = all_latents[n_train:n_train + n_valid]        # 中间10%
    test_data = all_latents[n_train + n_valid:]                # 最后10%

    np.save(f"{output_dir}/train_data.npy", train_data)
    np.save(f"{output_dir}/valid_data.npy", valid_data)
    np.save(f"{output_dir}/test_data.npy", test_data)

    print("\n--- CNF编码任务完成！---")
    print(f"训练数据已保存: {output_dir}/train_data.npy")
    print(f"验证数据已保存: {output_dir}/valid_data.npy")
    print(f"测试数据已保存: {output_dir}/test_data.npy")
    print(f"原始latent形状: {all_latents.shape}")
    print(f"训练集形状: {train_data.shape} (时间步 0-{n_train-1})")
    print(f"验证集形状: {valid_data.shape} (时间步 {n_train}-{n_train + n_valid-1})")
    print(f"测试集形状: {test_data.shape} (时间步 {n_train + n_valid}-{N_samples-1})")
    print(f"潜在向量维度: {hp.hidden_size} (与diffusion的latent_length保持一致)")
