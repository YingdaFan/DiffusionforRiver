...
class SIRENAutodecoder_film(nn.Module):
    ...
    #     # 先处理维度问题
    #     # 如果 coords 是 [1, 64, 30, 1]，我们需要重新理解数据结构
        
    #     if len(coords.shape) == 4:
    #         # coords: [1, batch_size, num_points, coord_dim] 或 [batch_size, 1, num_points, coord_dim]
    #         print(f"coords is 4D: {coords.shape}")
            
    #         # 尝试不同的重塑方式
    #         if coords.shape[0] == 1:
    #             # [1, batch_size, num_points, coord_dim] -> [batch_size, num_points, coord_dim]
    #             coords = coords.squeeze(0)
    #         elif coords.shape[1] == 1:
    #             # [batch_size, 1, num_points, coord_dim] -> [batch_size, num_points, coord_dim]
    #             coords = coords.squeeze(1)
            
    #         print(f"coords after squeeze: {coords.shape}")
            
    #     elif len(coords.shape) == 3:
    #         # coords: [batch_size, num_points, coord_dim] 或 [num_points, coord_dim, 1]
    #         print(f"coords is 3D: {coords.shape}")
            
    #         if coords.shape[-1] == 1:
    #             # [num_points, coord_dim, 1] -> [num_points, coord_dim]
    #             coords = coords.squeeze(-1)
    #             print(f"coords after squeeze: {coords.shape}")
        
    #     elif len(coords.shape) == 2:
    #         # coords: [num_points, coord_dim] - 这是我们期望的格式
    #         print(f"coords is 2D (expected): {coords.shape}")
            
    #         # 扩展到批次维度
    #         batch_size = latents.shape[0]
    #         coords = coords.unsqueeze(0).expand(batch_size, -1, -1)
    #         print(f"coords expanded: {coords.shape}")
        
    #     # 现在处理 latents
    #     if len(latents.shape) == 2:
    #         # latents: [batch_size, latent_dim]
    #         batch_size = latents.shape[0]
    #         num_points = coords.shape[1] if len(coords.shape) == 3 else coords.shape[0]
            
    #         # 扩展到空间维度
    #         expanded_latents = latents.unsqueeze(1).expand(-1, num_points, -1)
    #         print(f"latents expanded: {expanded_latents.shape}")
    #     else:
    #         print(f"Unexpected latents shape: {latents.shape}")
    #         expanded_latents = latents
        
    #     # 确保 coords 和 expanded_latents 的前两个维度匹配
    #     if coords.shape[:2] != expanded_latents.shape[:2]:
    #         print(f"Shape mismatch! coords: {coords.shape}, latents: {expanded_latents.shape}")
    #         # 尝试修复
    #         if len(coords.shape) == 2:
    #             coords = coords.unsqueeze(0).expand(expanded_latents.shape[0], -1, -1)
            
    #     print(f"Final coords shape: {coords.shape}")
    #     print(f"Final latents shape: {expanded_latents.shape}")
    ...
    #         print(f"After premap shape: {x.shape}")
        
    #     # 通过网络层处理
    #     for i in range(len(self.net1) - 1):
    #         print(f"\n--- Layer {i} ---")
    #         print(f"Input x shape: {x.shape}")
    #         print(f"Input latents shape: {expanded_latents.shape}")
            
    #         try:
    #             net1_output = self.net1[i](x)
    #             print(f"net1[{i}] output shape: {net1_output.shape}")
    #         except Exception as e:
    #             print(f"Error in net1[{i}]: {e}")
    #             print(f"net1[{i}] expects input dim: {self.net1[i].weight.shape}")
    #             raise e
                
    #         try:
    #             net2_output = self.net2[i](expanded_latents)
    #             print(f"net2[{i}] output shape: {net2_output.shape}")
    #         except Exception as e:
    #             print(f"Error in net2[{i}]: {e}")
    #             print(f"net2[{i}] expects input dim: {self.net2[i].weight.shape}")
    #             raise e
            
    #         x = net1_output + net2_output
    #         x = self.nl(x)
